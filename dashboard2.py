"""
Enhanced Options Pricing Anomaly Detection Dashboard

This dashboard visualizes anomalies detected in options pricing data with additional 
trading-focused information to make anomalies more actionable.
"""
...
def fetch_latest_prices() -> Dict:
    """Fetch latest prices for underlyings from API"""
    try:
        response = requests.get(f"{ANOMALY_API_URL}/api/market_data", timeout=3)
        if response.status_code == 200:
        ...
        print(f"Exception fetching prices: {str(e)}")
        return {}
