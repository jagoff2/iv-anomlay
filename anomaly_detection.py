...
class IVSurfaceAnomalyDetector(AnomalyDetector):
    ...
    def _fit_surface(self, data: List[OptionData], underlying: str) -> None:
        """Fit an implied volatility surface with robust error handling for 2D data"""
        ...
        try:
            ...
            # Check for degenerate dimension (near-zero variance)
            variances = np.var(X, axis=0)
            logger.debug(f"Feature variances for {underlying}: {variances}")
            
            # If any dimension has near-zero variance, we need a different approach
            if any(var < 1e-10 for var in variances):
                logger.warning(f"Detected near-zero variance in feature for {underlying}. Using robust model.")
                
                # Store data points for nearest-neighbor lookup
                self.surfaces[underlying] = (X, y, 'nearest')
                
                # Calculate mean and standard deviation for residuals
                mean_residual = 0.0  # Assume unbiased
                std_residual = max(0.02, np.std(y) * 0.2)  # Use fraction of IV std with minimum
                self.residuals[underlying] = (mean_residual, std_residual)
                
                # Store binned statistics for faster lookup
                from scipy.stats import binned_statistic
                
                # Bin by moneyness
                moneyness_bins = np.linspace(min(X[:, 0]) - 0.01, max(X[:, 0]) + 0.01, 10)
                bin_means, bin_edges, bin_numbers = binned_statistic(
                    X[:, 0], y, statistic='mean', bins=moneyness_bins
                )
                
                self.surfaces[underlying] = {
                    'type': 'binned',
                    'X': X,
                    'y': y,
                ...
                logger.info(f"Created binned model for {underlying} with {len(moneyness_bins)} bins")
                
            else:
                # Standard approach with multiple feature dimensions
                from sklearn.neighbors import KNeighborsRegressor
                
                # Use KNN regressor which is more robust than surface interpolation
                model = KNeighborsRegressor(n_neighbors=min(5, len(X)), weights='distance')
                model.fit(X, y)
                ...
                if len(X) >= 10:
                    ...
                    cv_predictions = cross_val_predict(model, X, y, cv=min(5, len(X)))
                    residuals = y - cv_predictions
                    mean_residual = np.mean(residuals)
                    std_residual = max(0.02, np.std(residuals))  # Ensure non-zero std
                    self.residuals[underlying] = (mean_residual, std_residual)
                    logger.info(f"Cross-validation residuals for {underlying}: mean={mean_residual:.4f}, std={std_residual:.4f}")
                else:
                    # Simple estimate if not enough for CV
                    std_residual = max(0.02, np.std(y) * 0.2)  # Use fraction of IV std
                    self.residuals[underlying] = (0.0, std_residual)
                    logger.info(f"Estimated residuals for {underlying}: std={std_residual:.4f}")
                
                logger.info(f"Created KNN model for {underlying} with {len(X)} points")
    ...
    def _predict_iv(self, data: List[OptionData], underlying: str) -> List[float]:
        """Predict implied volatility using robust approaches that avoid Qhull errors"""
        ...
        try:
            ...
            if isinstance(surface, dict):
                model_type = surface.get('type', 'unknown')
                
                if model_type == 'binned':
                    # Use binned statistics for prediction
                    valid_features = features[valid_indices]
                    valid_predictions = np.zeros(len(valid_features))
                    
                    # Find the appropriate bin for each point
                    for i, (m, _) in enumerate(valid_features):
                        bin_idx = np.digitize(m, surface['moneyness_bins']) - 1
                        bin_idx = max(0, min(bin_idx, len(surface['bin_means'])-1))
                        
                        # If bin has data, use it; otherwise use global mean
                        if not np.isnan(surface['bin_means'][bin_idx]):
                            valid_predictions[i] = surface['bin_means'][bin_idx]
                        else:
                            valid_predictions[i] = surface['global_mean']
                    ...
                    # Clip features to the training data range to avoid extrapolation
                    valid_features = features[valid_indices]
                    valid_features = np.clip(valid_features, X_min, X_max)
                    ...
                    # Unknown model type, use simple heuristic
                    logger.warning(f"Unknown model type: {model_type}")
                    predictions = self._fallback_prediction(features, valid_indices)
                    
            else:
                # Old tuple-based format, use fallback
                logger.warning(f"Legacy surface format detected for {underlying}")
                predictions = self._fallback_prediction(features, valid_indices)
            ...
            logger.debug(traceback.format_exc())
            return self._fallback_prediction(features, valid_indices)

    def _fallback_prediction(self, features: np.ndarray, valid_indices: np.ndarray) -> np.ndarray:
        """Fallback method for IV prediction when all else fails"""
        # Initialize predictions with NaNs
        ...
        for i in valid_idx:
            ...
            # Basic IV estimate based on moneyness
            if m < 0.85:  # Deep ITM
                base_iv = 0.30
            elif m > 1.15:  # Deep OTM
                base_iv = 0.40
            elif 0.95 <= m <= 1.05:  # Near ATM
                base_iv = 0.25
            else:  # Slightly ITM/OTM
                base_iv = 0.35
            ...
            # Volatility smile effect (quadratic)
            smile_adj = 1.0 + 0.5 * ((m - 1.0) ** 2)
            
            predictions[i] = base_iv * time_adj * smile_adj
